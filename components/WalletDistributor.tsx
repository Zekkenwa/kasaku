...
export default function WalletDistributor({ wallets, totalBalance, onClose }: Props) {
    ...
    return (
        <div className="space-y-5">
            {/* Total */}
            <div className="p-4 rounded-xl bg-[#458B73]/10 dark:bg-[#458B73]/20 transition-colors">
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">Total Saldo</p>
                <p className="text-2xl font-bold text-[#458B73] dark:text-[#458B73]">{currency(totalBalance)}</p>
            </div>
                    ...
                    <div key={w.id} className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 card-fix">
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[10px] px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400">{w.type}</span>
                            </div>
                        </div>
                        <input
                            type="number"
                        ...
                        <button onClick={() => handleDeleteWallet(w.id, w.name)}
                            className="w-7 h-7 rounded-full flex items-center justify-center text-gray-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/50 cursor-pointer text-sm flex-shrink-0">✕</button>
                    </div>
                ))}
            </div>
                ...
                <div className="p-3 rounded-xl border border-gray-200 dark:border-gray-700 space-y-2 bg-gray-50 dark:bg-gray-800/50">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button onClick={() => setShowAdd(false)}
                            className="px-4 py-2 rounded-lg text-sm text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">Batal</button>
                    </div>
                </div>
            ) : (
